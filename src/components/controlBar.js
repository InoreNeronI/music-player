...
class ControlBar extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.isPlaying !== this.props.isPlaying) {
      if (!this.props.isPlaying) this.stopTime();
      else this.startTime();
    }
  }
  startTime() {
  ...
  render() {
    const percentage = this.getPercentage(),
      ...
      endTime = getTime(this.props.tCount),
...
const PlayToggle = ({ isPlaying, togglePlaying, end }) => (
  <div
    id="play-toggle"
    className={isPlaying ? 'button toggled' : 'button'}
    onClick={() => {
      if (!end) {
...
const Volume = ({ mute, toggleMute }) => (
  <div
    id="volume-toggle"
    className={mute ? 'button toggled' : 'button'}
    onClick={() => {
      toggleMute(!mute);
    }}
  >
...
const Time = ({ time, endTime }) => (
  <div id="control-bar-time">
    <h1>
      <span id="current-time">{time}</span> / <span id="end-time">{endTime}</span>
    </h1>
  </div>
);
